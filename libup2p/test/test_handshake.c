...
test_read()
{
    ...
    while (tv->auth) {
        ...
        if ((cmp_q(rlpx_ch_remote_pub_ekey(&s.bob),
                   rlpx_ch_pub_ekey(&s.alice))))
            break;
        if ((cmp_q(rlpx_ch_remote_pub_ekey(&s.alice),
                   rlpx_ch_pub_ekey(&s.bob))))
            break;
...
test_write()
{
    ...
    IF_ERR_EXIT(
        rlpx_ch_auth_write(&s.alice, rlpx_ch_pub_skey(&s.bob), buf, &l));
