...
test_hex()
{
    ...
    IF_ERR_EXIT(uecc_qtob(rlpx_io_spub(s.alice), rawa, 65));
    IF_ERR_EXIT(uecc_qtob(rlpx_io_spub(s.bob), rawb, 65));
    IF_ERR_EXIT(rlpx_node_bin_to_hex(&rawa[1], 64, mema, NULL));
    IF_ERR_EXIT(rlpx_node_bin_to_hex(&rawb[1], 64, memb, NULL));
    IF_ERR_EXIT(check_q(rlpx_io_spub(s.alice), mema));
    IF_ERR_EXIT(check_q(rlpx_io_spub(s.bob), memb));
EXIT:
...
test_node()
{
    ...
    IF_ERR_EXIT(rlpx_node_init_enode(&node_alice, alice_pub));
    IF_ERR_EXIT(cmp_q(&node_alice.id, rlpx_io_spub(s.alice)));
    IF_ERR_EXIT((node_alice.port_tcp == 33) ? 0 : -1);
    IF_ERR_EXIT((node_alice.port_udp == 89) ? 0 : -1);
    IF_ERR_EXIT(cmp_q(&node_maxok.id, rlpx_io_spub(s.alice)));
